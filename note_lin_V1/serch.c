...
void writefile_preorder(const char *filename,struct node *T)		//前序写入文件 
{
	...
	while(top != -1)
	{
		...
		fprintf(fp,"%-2d%-50s%-120s%-20s%-20s%-20s%-10d\n",p->label,p->name,p->path,p->tag_1,p->tag_2,p->tag_3,p->time);
		if(p->P_right != NULL)
		{							//右子树的先进去 
			Stack[++top] = p->P_right;
		}
...
/*
struct node *create_right_txt(struct node *father,const char *name)	//增加同级结点_笔记文件 
{
	struct node *Node_Tree = (struct node *)malloc( sizeof(struct node) );
	Node_Tree->label = 1;
	sprintf(Node_Tree->path,father->path);
...
{											//需要输入指向树根的指针 
	...
	if(mkdir(mulu) == 0)
	{
		if( p != NULL) 
		{
			...
			{
				str = right_travel(p->P_left);			//找到右子树上最后一个结点
				create_right_label(p,mulu);				//创建
				printf("创建 %s 目录成功!\n",mulu);
				system("pause");
			}				
		}
...
/*	printf("————————————————\n");
	...
	writefile_inorder("preorder_tree.txt",father_Tree);
/**********************************************************/
	//结束操作 
	printf("关闭系统中.........");	
