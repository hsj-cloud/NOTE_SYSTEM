...
extern Tree HEAD;        //树的头节点
extern Tree POINTER;     //全局变量，实时指向用户打开的当前目录 
extern int top ;			//堆栈的变量 
extern FILE *f_write;	//写进txt文件时的指针 
extern FILE *f_read;	//读txt文件时的指针
...
/******************************************政佳的函数*********************************************/ 
int remove_mulu(const char *dir);			//删除目录 
struct node *getTop(struct node **a);		//拿到栈顶元素
void pop( );								//弹栈函数
...
char *divide_filename(const char *path);		//分隔路径函数
struct node *find_first(struct node *father_Tree,struct node *Node_Tree,const char *path);//利用路径找到结点的第一个父结点
...
void perorder(struct node *T);				//前序遍历打印 
