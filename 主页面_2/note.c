...
void Read_Ordinary_users(LinkList *L)  //读入文本文档（Ordinary_users.txt）
{
	LinkList p,r;
	FILE *file_p_read;
	file_p_read=fopen("C:\\Users\\HSJ\\Desktop\\NOTE_SYSTEM\\FILE_普通用户_账号_密码\\Ordinary_users.txt","r");
	r=*L;  //r为指向尾部的结点
	int i;
...
void Updata_Ordinary_users(LinkList *L)  //更新普通用户数据到文本文档（Ordinary_users.txt）
{
	...
	LinkList p,q;
	FILE *file_p_write;
	file_p_write=fopen("C:\\Users\\HSJ\\Desktop\\NOTE_SYSTEM\\FILE_普通用户_账号_密码\\Ordinary_users.txt","w");
	p=*L;  //p为指向头部的结点
	p=p->next;
	while(p)
	{
...
void create(void){  //创建txt文件
	...
	fclose(fp);  
	Tree pNew=(node*)malloc(sizeof(node));
	pNew->label = 1;
