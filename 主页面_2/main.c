			...
			if((Login_system(&Ordinary_users))==1)  //调用登录系统函数 
			{
				...
				switch(getchar())
				{
					...
					case'7':
						...
						{
							display_ls_NotePath(BiTree T,new_data->content_1);  //显示指令三：ls <笔记文件夹路径>（显示指定文件下所有的内容（子笔记文件夹和笔记文件））
						}
						if(new_data->value_1==4)
						{
							display_ls_NotePath_grep(BiTree T,new_data->content_1,new_data->content_2);  //显示指令四：ls <笔记文件夹路径> grep "搜索内容" （显示命令指定文件下所有的带有搜索内容的文件夹名及文件名）
						}
						if(new_data->value_1==100)
						{
							system("cls");  //清屏
